...
/**
 * Callback OnlyOffice Slides untuk presentasi (#226). Mirror dari
 * OnlyOfficeCallbackController (memo) namun terisolasi di namespace presentasi
 * dengan key bersegmen `presentation-{id}-`. Menyimpan PPTX terbaru dari editor
 * ke private disk dan menandai versi aktif sebagai edited.
 */
class PresentationCallbackController extends Controller
{
    public function __invoke(Request $request, Presentation $presentation): JsonResponse
    {
        ...
        $version = $this->resolvePresentationVersion($request, $presentation, $callback);
        ...
        // ── Status 4: tidak ada editor aktif (close/no-change) ──────────────
        if ($status === 4) {
        ...
        if (in_array($status, [2, 6], true)) {
            ...
            try {
                ...
                $lock->block(10, function () use ($presentation, $version, $path, $pptxBytes, $callback, $replayCacheKey, $status) {
                    if (Cache::has($replayCacheKey)) {
                        abort(Response::HTTP_CONFLICT, 'Callback OnlyOffice sudah diproses (anti-replay).');
                    }
                    ...
                    if ($version) {
                        $version->forceFill([
                            'pptx_path' => $path,
                            'status' => Presentation::STATUS_EDITED,
                        ])->save();

                        if ((int) $presentation->current_version_id === (int) $version->id || $presentation->current_version_id === null) {
                        ...
                        $currentVersion = $presentation->currentVersion()->first();
     ...
     * @param  array<string, mixed>  $callback
     */
    protected function resolvePresentationVersion(Request $request, Presentation $presentation, array $callback): ?PresentationVersion
    {
