...
>
    ...
    <div class="flex items-center justify-between px-4 pb-2 pt-3">
        <a href="{{ route('dashboard') }}" wire:navigate @click="showMemoSidebar = false" class="inline-flex items-center px-1 py-2.5 font-medium text-[13px] text-gray-700 dark:text-gray-200 hover:text-amber-800 dark:hover:text-amber-300 transition-colors duration-200">
    ...
    <div class="p-4 pt-2 pb-5">
        <button type="button"
            ...
            <svg wire:loading.remove wire:target="startNewMemo" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-[#64748B] dark:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 5v14m-7-7h14" />
            </svg>
            <span wire:loading.inline-flex wire:target="startNewMemo" class="mr-2 h-4 w-4 rounded-full border-2 border-current/40 border-t-transparent animate-spin text-ista-primary" aria-hidden="true"></span>
            Memo Baru
        </button>
    </div>
                    ...
                    @forelse ($groupMemos as $memo)
                        <li class="group relative" wire:key="memo-sidebar-{{ $groupKey }}-{{ $memo->id }}" x-show="isMemoVisible(@js($memo->title))">
                            ...
                            <button type="button"
                                ...
                                <svg wire:loading.remove wire:target="deleteMemo" xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                                <span wire:loading.inline-flex wire:target="deleteMemo" class="h-3.5 w-3.5 rounded-full border border-current border-t-transparent animate-spin" aria-hidden="true"></span>
                            </button>
                        </li>
                    @empty
                        @if ($groupKey === 'today')
    ...
    <div class="px-4 py-6 text-sm flex flex-col gap-2">
        <a href="{{ route('profile', ['from' => 'memo']) }}" wire:navigate class="flex items-center gap-3 text-gray-700 dark:text-[#F8FAFC] hover:opacity-80 transition-opacity">
