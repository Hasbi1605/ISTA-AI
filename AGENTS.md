...
## Changelog
- 2026-06-20 — Fase 5 epic Presentasi (#226): OnlyOffice Slides Browser Editor. Tambah versioning presentasi (`presentation_versions` + `PresentationVersion` + relasi `versions()`/`currentVersion()`; `GeneratePresentation` mencatat versi aktif tiap sukses). `PresentationDocumentKey` diperluas: editor key stabil per-sesi (anti-stale), `signedFileUrl` dengan oo_token single-use + version, `forEditor`/`invalidateEditorKey`. Tambah `PptxValidator` (OOXML presentation), `PresentationOnlyOfficeCallbackController` (`POST /onlyoffice/presentation-callback/{presentation}`: JWT verify, key prefix `presentation-`, fresh-key check, trusted URL + anti path-traversal, anti-replay, persist editan ke versi aktif + invalidasi PDF), `PresentationForceSaveService` (mirror memo). `PresentationFileController` kini sadar versi + oo_token single-use di signed endpoint + endpoint `force-save`; `PresentationConverter` sadar versi. UI: aksi **Edit Presentasi** + editor OnlyOffice Slides di `PresentationWorkspace` (`editorConfig()` documentType=slide, `closeEditor()` force-save). Namespace presentasi dipisah dari memo (key/secret/cache/callback berbeda); auth/signed-URL/callback host check/private storage tidak dilonggarkan. Memo/chat tidak diubah. — `laravel/database/migrations/2026_06_20_000002_create_presentation_versions_table.php`, `laravel/app/Models/{Presentation,PresentationVersion}.php`, `laravel/app/Jobs/GeneratePresentation.php`, `laravel/app/Services/OnlyOffice/{PresentationDocumentKey,PresentationConverter,PptxValidator,PresentationForceSaveService}.php`, `laravel/app/Http/Controllers/Presentations/{PresentationFileController,PresentationOnlyOfficeCallbackController}.php`, `laravel/app/Services/Presentations/PresentationLifecycleService.php`, `laravel/app/Livewire/Presentations/PresentationWorkspace.php`, `laravel/resources/views/livewire/presentations/presentation-workspace.blade.php`, `laravel/routes/web.php`, `laravel/tests/Feature/Presentations/*`, `docs/CODEBASE-CONTEXT.md` — (test: belum dijalankan di sandbox — PHP/composer tidak tersedia; jalankan `cd laravel && php artisan test --filter='Presentation|OnlyOffice'` lalu full PHPUnit sebelum merge)
