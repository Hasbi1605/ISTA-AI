...
- **Presentations/** `PresentationFileController` (signed/download PPTX/exportPdf/forceSave), `PresentationOnlyOfficeCallbackController` (`POST /onlyoffice/presentation-callback/{presentation}`: editor OnlyOffice Slides, persist PPTX editan ke versi aktif, invalidasi PDF cache; namespace terpisah dari memo).
...
- **Presentations/** `PresentationWorkspace` (mode Presentasi epic #218: sub-mode `Buat PPT ISTA` form konfigurasi hybrid + pilih dokumen ready + status/history + download PPTX/PDF; **Edit Presentasi** membuka PPTX ready di OnlyOffice Slides (`editorConfig()` `documentType=slide`/`fileType=pptx`), `closeEditor()` force-save sebelum tutup (#226); sub-mode `Prompy Studio` placeholder untuk #263).
...
- **OnlyOffice/** `DocumentConverter`, `DocxTextExtractor`, `DocxValidator`, `JwtSigner`, `MemoDocumentKey`, `MemoForceSaveService`, `ForceSaveException`, `PresentationConverter` (PPTX→PDF, sadar versi), `PresentationDocumentKey` (signed internal URL PPTX + editor key stabil per-sesi + oo_token single-use + versioning, #226), `PptxValidator` (validasi OOXML presentation sebelum simpan), `PresentationForceSaveService` (force-save editor Slides, mirror `MemoForceSaveService`).

### 2.4 Models / Jobs / Middleware
- **Models:** `User`, `Conversation`, `Message`, `Document`, `DocumentChunk`, `Memo`, `MemoVersion`, `Presentation` (artefak presentasi: status pending/processing/ready/error, configuration/outline/source_document_ids JSON, `current_version_id` ke versi aktif, relasi `versions()`/`currentVersion()`, helper `sanitizeSourceDocumentIds()`/`sourceDocumentsOwnedAndReady()` fail-closed owned+ready), `PresentationVersion` (versi PPTX per presentasi: version_number, pptx_path, status generated/edited — #226), `KnowledgeDocument`, `KnowledgeChunk`, `KnowledgeSource`, `AIUsageEvent`, `AdminAccountAudit`.
...
- **Jobs:** `GenerateChatResponse` (jalur async chat, komplemen SSE), `ProcessDocument` (→ python `/api/documents/process`), `ProcessKnowledgeDocument` (→ `/api/knowledge/process`, memakai `processing_claim_token` agar retry lama tidak menimpa attempt baru), `RenderDocumentPreview`, `GeneratePresentation` (async generate PPTX → python `/api/presentations/generate`, claim-token guard pending→processing→ready/error; setiap sukses mencatat `PresentationVersion` baru sebagai versi aktif — #226).
...
Tabel utama: `users` (+verification_code, role/presence, kolom keamanan admin), `conversations`, `messages` (+document_ids), `documents` (metadata file, preview, index, unique original_name), `document_chunks`, `memos` (+chat_messages, configuration), `memo_versions`, `presentations` (artefak presentasi: status, visual_template, configuration/outline/source_document_ids JSON, pptx_path/pdf_path, generated_at, current_version_id ke versi aktif, softDeletes), `presentation_versions` (versi PPTX per presentasi: version_number unik per presentasi, pptx_path, status generated/edited — #226 OnlyOffice Slides), `ai_usage_events`, knowledge (`knowledge_sources`/`knowledge_documents`/`knowledge_chunks`), `admin_account_audits`, `activity_log` (spatie) + cache/jobs. Migrasi lama menghapus tabel `ai_configuration` (config AI sekarang YAML-driven) serta `cloud_storage_files` dan `google_drive_oauth_connections` (integrasi Google Drive sudah dihapus total). Seeder: `DatabaseSeeder`; factory: `UserFactory`.
...
- **Presentasi:** `PresentationWorkspace` → `PresentationGenerationService`/`GeneratePresentation` → python `/api/presentations/generate` (PPTX + `PresentationVersion`). Edit PPTX di OnlyOffice Slides (`PresentationOnlyOfficeCallbackController` + JWT + key `presentation-` + URL trusted) dengan force-save sebelum tutup/unduh; editan disimpan ke versi aktif dan PDF cache di-invalidasi sehingga unduh PPTX/PDF memakai versi terbaru (#226).
