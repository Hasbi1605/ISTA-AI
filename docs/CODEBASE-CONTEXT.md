...
- **Presentations/** `PresentationGenerationService` (createAndDispatch + render PPTX via python `/api/presentations/generate`, outline deterministik dari konfigurasi, simpan ke private disk; job `GeneratePresentation` memakai konfigurasi `presentations.queue.*` sehingga production mengirim render PPT ke Redis/Horizon secara eksplisit; `normalizeConfiguration` menormalkan **`asset_mode`** ke `local_assets_only`/`licensed_web_assets` — default lokal #227 — dan meneruskannya ke payload renderer via `normalizeAssetMode()`), `PresentationLifecycleService` (hapus presentasi + artefak).
