...
- **Chat/** `ChatDocumentStateService` (metadata dokumen sidebar + cache TTL pendek), `ChatPendingRefreshNotifier` (cache signal refresh pending chat setelah job `GenerateChatResponse` selesai persist).
...
- **js/**: `app.js` (bootstrap + viewport sync; **dynamic import** `chat-page.js` hanya bila `[data-ista-chat-shell]` ada di halaman `/chat`), `bootstrap.js`, **`chat-page.js`** (chunk terpisah: Alpine `chatLayout` tab chat/memo/Prompy (`prompyEnabled` gating tab; drag-drop global **hanya tab chat** lewat event `chat-files-drop` → antre upload berurutan maks. 5/drop + kuota 10 dokumen; helper `istaAssignFilesToInput`/`istaMergeFilesWithCap`/`istaClassifyPrompyDroppedFiles`), `prompyWorkspace`, `assistantTypewriter`, `chatMessages` dengan **smart auto-scroll** (`stickToBottom` + tombol lompat ke bawah), **stop generation** (tombol Stop di composer → `ChatIndex::stopGeneration()` persist partial + flag cache agar SSE `done` idempoten), **regenerate/edit user message** (`ChatIndex::regenerate()` / `editUserMessage()` truncate + redispatch job + buka SSE ulang), markdown code block + copy + lazy `ensureIstaHljs()`, table overflow wrapper, suggestion chips empty state; SortableJS di-load lazy via `window.ensureIstaSortable()` saat reorder gambar Prompy — reorder tidak re-upload Livewire, sync file input hanya saat add/remove/submit). **SSE**: `openChatStream(...)` membangun `/chat/stream/{id}?...` lalu `new EventSource(url)` (dilacak di `activeEventSources`), render typewriter markdown (marked + DOMPurify); koordinasi placeholder via `$wire.on` (`assistant-output`, `model-name`, `assistant-sources`, dll) + marker localStorage.
- **css/**: `fonts.css` (self-host via `@fontsource/*` **latin-only**: Inter, EB Garamond, Fraunces/Plus Jakarta Sans variable latin subset; Manrope/Instrument Sans dihapus), `fonts-variable-latin.css`, `app.css` (shell user/chat ~300 baris + motion tokens `--ista-ease`/`--ista-duration-*`, utility `.ista-transition*`), `admin.css` (design system admin terpisah, hanya di-load layout admin), `auth.css`. Komponen `<x-font-preloads />` preload woff2 kritis pasca-build.
- **navigasi SPA**: `wire:navigate` pada CTA dashboard, link beranda/profil sidebar chat/memo/prompy, dan nav layout; riwayat chat tetap client-side via Alpine `navigateToConversation`/`loadConversation` (bukan full reload). `<x-page-loader />` mendengarkan `livewire:navigating`/`livewire:navigated`. Script anti-FOUC dark mode di `layouts/app` + dashboard re-apply class saat `livewire:navigated`.
- **polling gating**: chat poll 5s/60s hanya bila `pendingConversationIds` tidak kosong; sidebar dokumen poll 5s/20s via `hasDocumentsInProgress`; admin knowledge poll 5s via `shouldPollKnowledgePipeline`; admin users poll 30s via `shouldPollUserPresence`; document viewer poll preview/keep-alive via flag eksplisit saat modal terbuka.
...
- `chroma_store.py` — cache process-local instance `Chroma` per `(collection, embedding, persist_directory)` untuk hot path retrieval.
- `rag_retrieval.py` — `search_relevant_chunks` (vector + BM25 child corpus), filter scope user/document_ids; memakai `get_chroma_store`.
...
### 5.1 Flow User
- **Chat (streaming):** `ChatIndex` persist pesan user → `chat-page.js` buka `EventSource /chat/stream/{id}` → `ChatStreamController` rebuild history + resolve konteks/policy → `AIService::sendChat()` → python `/api/chat`. Python memutuskan: RAG-with-sources (dokumen aktif) / fallback web-search / knowledge internal / general chat, stream token balik. Controller sanitasi + persist idempoten, emit `model-name`/`sources`/`final-content`/`message-id`/`done`; client render typewriter + sumber + auto-scroll pintar (stick-to-bottom + jump-to-latest), tombol **Stop** saat streaming (`stopGeneration` persist partial + flag `chat:stream-stopped`), aksi **Coba lagi** (`regenerate`) dan **edit pesan user** (truncate + redispatch), code block copy + highlight lazy, suggestion chips empty state. Usage → `ai_usage_events`. UI chat menampilkan window 50 pesan terakhir + tombol "Muat pesan lebih lama"; history ke Python/job tetap dibangun server-side dari DB (window `max_history_messages`, default 20). Jalur fallback job `GenerateChatResponse` memanggil `ChatPendingRefreshNotifier::signal()` setelah persist assistant/error; UI menunggu pending lewat poll ringan `pollPendingChatSignals` (5s) dan reconcile `refreshPendingChatState` (60s). Sidebar dokumen memakai cache metadata TTL pendek via `ChatDocumentStateService`.
