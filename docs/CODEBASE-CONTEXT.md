...
- **Presentations/** `PresentationFileController` (download PPTX/PDF owner-only, signed-file untuk OnlyOffice + oo_token single-use, forceSave sebelum unduh), `PresentationCallbackController` (`POST /onlyoffice/presentation-callback/{presentation}`: simpan PPTX terbaru dari OnlyOffice Slides, JWT + key segar + trusted URL + anti-replay + validasi PPTX).
...
- **Chat/** `ChatIndex` (workspace chat+memo+presentasi bertab, route `chat/{id?}`; toggle 3 mode dengan tab Presentasi aktif default via `features.presentation` dan bisa di-rollback dengan `FEATURE_PRESENTATION=false`; `normalizeTab()` memetakan alias `presentasi`→`presentation` dan menormalkan nilai invalid/tab presentasi saat flag mati ke `chat`).
- **Memos/** `MemoIndex`, `MemoWorkspace` (generate/edit memo).
- **Presentations/** `PresentationWorkspace` (mode Presentasi epic #218: shell 3 panel seperti Memo — sidebar riwayat compact, panel konfigurasi, dan panel kanan permanen untuk status/render/editor; sub-mode `Buat PPT ISTA` form konfigurasi hybrid + pilih dokumen ready + status/history + download PPTX/PDF; presentasi ready otomatis membuka editor OnlyOffice Slides (`documentType=slide`) via `editorConfig()` di panel kanan; pending/processing yang tidak berubah >10 menit tampil ringkas dengan aksi kirim ulang job dan juga diselesaikan otomatis oleh scheduler menjadi `error` bila tidak ada active claim; sub-mode `Prompy Studio` me-embed komponen `PrompyStudio`). `PrompyStudio` (#263: generator paket prompt untuk platform AI eksternal — sidebar kiri khusus riwayat prompt private dengan status selesai, form di panel tengah, dan hasil paket prompt/copy actions di panel kanan; input ide Bahasa Indonesia, pilih platform/jenis, dokumen sumber ready opsional (cuplikan `document_chunks` dibatasi dan dikirim sebagai konteks internal) + reference image privat (file disimpan lokal, Python hanya menerima flag); output prompt utama Bahasa Inggris + catatan Bahasa Indonesia; header/riwayat memakai judul ringkas deterministik dari ide; copy action main/variants/negative/settings; state terpisah dari generate PPTX, tidak memanggil platform eksternal).
...
- **Presentations/** `PresentationGenerationService` (createAndDispatch + render PPTX via python `/api/presentations/generate`, outline deterministik dari konfigurasi, simpan ke private disk; production default `PRESENTATION_GENERATION_MODE=inline` menjalankan `GeneratePresentation` langsung saat submit/retry agar tidak bergantung pada worker queue, sedangkan mode `queued` tetap tersedia dan memakai konfigurasi `presentations.queue.*`; `normalizeConfiguration` menormalkan **`asset_mode`** ke `local_assets_only`/`licensed_web_assets` — default lokal #227 — dan meneruskannya ke payload renderer via `normalizeAssetMode()`), `PresentationLifecycleService` (hapus presentasi + artefak).
- **Prompts/** `PromptStudioService` (#263: generate paket prompt via python `/api/prompts/generate`, validasi platform/jenis & dokumen sumber owned+ready, membangun `source_context` terbatas dari `document_chunks`, simpan reference image privat dengan validasi tipe/ukuran, set `contains_internal_context`, log usage `prompt_generation`; tidak memanggil platform AI eksternal).
- **OnlyOffice/** `DocumentConverter`, `DocxTextExtractor`, `DocxValidator`, `JwtSigner`, `MemoDocumentKey`, `MemoForceSaveService`, `ForceSaveException`, `PptxValidator` (validasi struktur PPTX dari callback), `PresentationConverter` (PPTX→PDF, mendukung versi aktif), `PresentationDocumentKey` (editor key per-sesi + signed internal URL + oo_token single-use PPTX), `PresentationForceSaveService` (force-save OnlyOffice Slides sebelum unduh).

### 2.4 Models / Jobs / Middleware
- **Models:** `User`, `Conversation`, `Message`, `Document`, `DocumentChunk`, `Memo`, `MemoVersion`, `Presentation` (artefak presentasi: status pending/processing/ready/error/edited, configuration/outline/source_document_ids JSON, `current_version_id`, relasi `versions()`/`currentVersion()`, helper `sanitizeSourceDocumentIds()`/`sourceDocumentsOwnedAndReady()` fail-closed owned+ready), `PresentationVersion` (versi PPTX presentasi untuk editor OnlyOffice Slides #226), `KnowledgeDocument`, `KnowledgeChunk`, `KnowledgeSource`, `AIUsageEvent`, `AdminAccountAudit`.
...
- **Jobs:** `GenerateChatResponse` (jalur async chat, komplemen SSE), `ProcessDocument` (→ python `/api/documents/process`), `ProcessKnowledgeDocument` (→ `/api/knowledge/process`, memakai `processing_claim_token` agar retry lama tidak menimpa attempt baru), `RenderDocumentPreview`, `GeneratePresentation` (generate PPTX → python `/api/presentations/generate`, claim-token guard pending→processing→ready/error; dipakai inline oleh service di production dan bisa kembali async bila `PRESENTATION_GENERATION_MODE=queued`, dengan `PRESENTATION_QUEUE_CONNECTION=redis` agar sejajar dengan Horizon). Command `presentations:resolve-stale-renders` menandai render presentasi pending/processing lama sebagai `error` agar tidak menggantung tanpa feedback; command `presentations:recover-stale-renders` me-redispatch render stale tanpa active claim secara terbatas, dipakai deploy untuk memulihkan job yang tersangkut saat worker/queue bermasalah.
...
- **web.php:** dashboard `/`; route user seperti `profile`, `chat/{id?}` (ChatIndex), `chat/stream/{conversationId}` (SSE), memo, dan dokumen memakai `auth+active` (serta `verified` sesuai kebutuhan) agar akun nonaktif tidak bisa memakai aplikasi lewat sesi lama; `onlyoffice/callback/{memo}` (JWT + trusted OnlyOffice URL scheme/host/port); grup `chat/memos/*` (download/export-pdf/force-save/signed); legacy `memos/*` redirect ke `/chat?tab=memo`; `onlyoffice/presentation-callback/{presentation}` (JWT + key segar + trusted URL + anti-replay, simpan PPTX dari OnlyOffice Slides); grup `chat/presentations/*` (auth+active+verified): `download` (PPTX) & `export-pdf` & `force-save`, plus `chat/presentations/{presentation}/signed-file` (tanpa auth, dipakai OnlyOffice converter/editor; wajib signature HMAC + oo_token single-use); `documents/*` (content-html, extract-tables, export, preview/*); admin auth (`/admin/login`, logout, password change); admin app (`auth+verified+admin+admin.password_changed`): `/admin`, `/users`, `/usage`, `/errors`, `/documents`, `/knowledge`; super-admin: `/admin/accounts`.
...
- **console.php:** `documents:purge-deleted --days=7` (03:00), `chat:resolve-stale-responses --minutes=10` (tiap menit), `presentations:resolve-stale-renders --minutes=10` (tiap menit).

### 2.6 Frontend Laravel (`resources/`)
- **js/**: `app.js` (registrasi Alpine+Livewire), `bootstrap.js`, **`chat-page.js`** (client berat: Alpine `chatLayout` tab chat/memo/presentasi (`presentationEnabled` gating tab Presentasi), `presentationWorkspace` untuk sidebar/mobile panel/loading phase Presentasi/Prompy, drag-drop upload, `assistantTypewriter`, `chatMessages`). **SSE**: `openChatStream(...)` membangun `/chat/stream/{id}?...` lalu `new EventSource(url)` (dilacak di `activeEventSources`), render typewriter markdown (marked + DOMPurify); koordinasi placeholder via `$wire.on` (`assistant-output`, `model-name`, `assistant-sources`, dll) + marker localStorage.
- **css/**: `app.css`, `auth.css`.
- **views/**: `dashboard` (CTA Chat/Memo + Presentasi saat `features.presentation` aktif), `profile`; `livewire/chat/*` (+partials), `livewire/memos/*`, `livewire/presentations/*` (+partials shell riwayat/konfigurasi/preview), `livewire/documents/*`, `livewire/admin/*` (dashboard/users/usage/errors/documents/knowledge/accounts), Volt auth pages, `components/admin/*` (design system: sidebar/table/tabs/kpi-card/badge/filter/empty-state), `layouts/` (app/admin/guest/auth-canvas), `emails/`.

### 2.7 Database (`laravel/database`)
Tabel utama: `users` (+verification_code, role/presence, kolom keamanan admin), `conversations`, `messages` (+document_ids), `documents` (metadata file, preview, index, unique original_name), `document_chunks`, `memos` (+chat_messages, configuration), `memo_versions`, `presentations` (artefak presentasi: status, visual_template, configuration/outline/source_document_ids JSON, pptx_path/pdf_path, generated_at, current_version_id menunjuk versi aktif, softDeletes), `presentation_versions` (versi PPTX per presentasi: version_number, label, pptx_path, status; dipakai editor OnlyOffice Slides #226), `generated_prompts` (riwayat Prompy Studio #263: platform/prompt_type + label, title, idea, package JSON, source_document_ids JSON, contains_internal_context, reference_image_path/mime/size privat, model_label, owner-scoped + softDeletes), `ai_usage_events`, knowledge (`knowledge_sources`/`knowledge_documents`/`knowledge_chunks`), `admin_account_audits`, `activity_log` (spatie) + cache/jobs. Migrasi lama menghapus tabel `ai_configuration` (config AI sekarang YAML-driven) serta `cloud_storage_files` dan `google_drive_oauth_connections` (integrasi Google Drive sudah dihapus total). Seeder: `DatabaseSeeder`; factory: `UserFactory`.
...
- `presentations.py` — `/api/presentations/generate` (POST, binary PPTX) + `/api/presentations/templates` (GET): renderer presentasi **deterministic** (epic #218/#221), bukan AI. Pydantic `GeneratePresentationRequest` (title, visual_template, outline[], header/footer/presenter/unit/slide_count/audience, **`asset_mode`** + **`licensed_assets[]`** opsional #227), `ValueError`→400, header `X-Presentation-Template`/`X-Presentation-Slide-Count`/**`X-Presentation-Asset-Mode`**/**`X-Presentation-Licensed-Assets`** + `nosniff`/`no-store`. `/templates` juga mengembalikan `asset_modes`, `default_asset_mode`, dan `licensed_providers` (whitelist). Pengayaan aset web berlisensi (#227) bersifat **opsional & default mati**: tanpa fetcher jaringan tidak ada koneksi dan semua kandidat fallback ke aset lokal.
- `prompts.py` — `/api/prompts/generate` (POST, JSON) + `/api/prompts/profiles` (GET): Prompy Studio (#263). Memanggil LLM untuk menyusun paket prompt (main_prompt EN, variants, negative_prompt, recommended_settings, notes_id ID) untuk platform AI eksternal; payload dapat membawa `source_context` dari cuplikan dokumen internal terpilih; **tidak** memanggil platform tersebut. Profil platform/jenis & template dari `ai_config.yaml` (`prompt_studio`). `ValueError`→400.
...
- `lightweight_text_splitter.py` — splitter token-aware. `document_loaders.py` — load pdf/docx/xlsx/csv→Document. `document_content.py` — dokumen→HTML (preview/export). `table_extraction.py` — ekstrak tabel. `document_export.py` — `export_content` (HTML→format target). `memo_generation.py` — `generate_memo_docx`. `prompt_generation.py` — `generate_prompt_package` (Prompy Studio #263: rangkai paket prompt JSON via LLM, parse defensif, memakai `source_context` dokumen internal bila dikirim Laravel, fallback platform/jenis generic/image). `presentation_render.py` — `render_presentation` (outline→PPTX 16:9 deterministic, 5 template visual, header/footer/logo, validasi template/slide/bullet) + `presentation_assets.py` (logo emblem + registry ikon lokal, `ASSET_MODE='local_assets_only'`, no-internet; baseline QA visual di `docs/presentation-visual-qa.md`) + `presentation_web_assets.py` (**#227** licensed web asset enrichment opsional: `LicensedWebAssetService` dengan provider whitelist `LICENSED_PROVIDERS` (Openverse, Wikimedia Commons), kebijakan lisensi `ALLOWED_LICENSES` (CC0/Public Domain/CC-BY/CC-BY-SA), validasi metadata wajib source/license/attribution, cache privat `storage/presentation_assets/` + sha256, fallback ke lokal saat provider/jaringan/lisensi gagal, dan `AssetAuditTrail`. Default no-internet: tanpa fetcher disuntik, `enrich()` selalu `None`). `latency_logger.py` — timing privacy-safe.
...
- **Presentasi:** `PresentationWorkspace` → `PresentationGenerationService` → python `/api/presentations/generate` (PPTX + `PresentationVersion`). UI memakai shell 3 panel: sidebar riwayat compact, konfigurasi tengah, panel kanan permanen untuk empty/loading/stale/error/editor. Production default menjalankan render inline saat submit/retry sehingga presentasi bisa langsung menjadi `ready` dan editor terbuka tanpa menunggu Horizon; mode queued tetap opsional untuk rollback/operasional queue. Presentasi ready otomatis membuka OnlyOffice Slides (`documentType=slide`) dengan `editorConfig()` (signed file URL + oo_token + JWT); pending/processing lama (>10 menit tanpa update) diberi recovery action untuk redispatch job dan scheduler `presentations:resolve-stale-renders` menutup stale render menjadi `error` bila tidak ada active claim. Persist edit lewat `PresentationCallbackController` (key segar + trusted URL + anti-replay + validasi PPTX). Download PPTX/PDF (`download`/`export-pdf`) memicu force-save (`force-save`) dulu agar memakai versi aktif terbaru.
- **Prompy Studio:** sub-mode Presentasi yang memakai `PrompyStudio` untuk menyusun paket prompt eksternal. Sidebar kiri berisi riwayat prompt private yang sudah berhasil dibuat, panel tengah berisi form ide/platform/jenis/dokumen sumber, dan panel kanan hanya menampilkan paket prompt aktif beserta tombol salin. Judul prompt dipadatkan deterministik dari ide agar header tidak menyalin permintaan user penuh.
...
- Callback OnlyOffice memakai JWT ber-`exp`, key memo/presentasi/version segar, anti-replay, validasi DOCX/PPTX, dan URL download yang harus cocok scheme/host/port trusted. Signed file URL presentasi (editor/converter) wajib signature HMAC origin internal + oo_token presentasi-bound single-use; akses anonim dari host publik ditolak.
