...
- **Presentations/** `PresentationWorkspace` (mode Presentasi epic #218: shell 3 panel seperti Memo — sidebar riwayat compact, panel konfigurasi, dan panel kanan permanen untuk status/render/editor; sub-mode `Buat PPT ISTA` form konfigurasi hybrid + pilih dokumen ready + status/history + download PPTX/PDF; presentasi ready otomatis membuka editor OnlyOffice Slides (`documentType=slide`) via `editorConfig()` di panel kanan; pending/processing yang tidak berubah >10 menit tampil ringkas dengan aksi kirim ulang job dan juga diselesaikan otomatis oleh scheduler menjadi `error` bila tidak ada active claim; sub-mode `Prompy Studio` me-embed komponen `PrompyStudio`). `PrompyStudio` (#263: generator paket prompt untuk platform AI eksternal — form di panel tengah, hasil paket prompt + riwayat/copy actions di panel kanan; input ide Bahasa Indonesia, pilih platform/jenis, dokumen sumber ready opsional (cuplikan `document_chunks` dibatasi dan dikirim sebagai konteks internal) + reference image privat (file disimpan lokal, Python hanya menerima flag); output prompt utama Bahasa Inggris + catatan Bahasa Indonesia; copy action main/variants/negative/settings; riwayat private; state terpisah dari generate PPTX, tidak memanggil platform eksternal).
