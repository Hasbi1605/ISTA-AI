...
- **Presentations/** `PresentationGenerationService` (createAndDispatch + render PPTX via python `/api/presentations/generate`, outline deterministik dari konfigurasi, simpan ke private disk; `normalizeConfiguration` menormalkan **`asset_mode`** ke `local_assets_only`/`licensed_web_assets` — default lokal #227 — dan meneruskannya ke payload renderer via `normalizeAssetMode()`), `PresentationLifecycleService` (hapus presentasi + artefak).
...
- **Jobs:** `GenerateChatResponse` (jalur async chat, komplemen SSE), `ProcessDocument` (→ python `/api/documents/process`), `ProcessKnowledgeDocument` (→ `/api/knowledge/process`, memakai `processing_claim_token` agar retry lama tidak menimpa attempt baru), `RenderDocumentPreview`, `GeneratePresentation` (async generate PPTX → python `/api/presentations/generate`, claim-token guard pending→processing→ready/error). Command `presentations:resolve-stale-renders` menandai render presentasi pending/processing lama sebagai `error` agar tidak menggantung tanpa feedback.
