...
- **Presentations/** `PresentationWorkspace` (mode Presentasi epic #218: sub-mode `Buat PPT ISTA` form konfigurasi hybrid + pilih dokumen ready + status/history + download PPTX/PDF; aksi `Edit Presentasi` membuka editor OnlyOffice Slides (`documentType=slide`) untuk presentasi ready via `editorConfig()`; sub-mode `Prompy Studio` me-embed komponen `PrompyStudio`). `PrompyStudio` (#263: generator paket prompt untuk platform AI eksternal — input ide Bahasa Indonesia, pilih platform/jenis, dokumen sumber ready opsional (cuplikan `document_chunks` dibatasi dan dikirim sebagai konteks internal) + reference image privat (file disimpan lokal, Python hanya menerima flag); output prompt utama Bahasa Inggris + catatan Bahasa Indonesia; copy action main/variants/negative/settings; riwayat private; state terpisah dari generate PPTX, tidak memanggil platform eksternal).
...
- **views/**: `dashboard` (CTA Chat/Memo + Presentasi saat `features.presentation` aktif), `profile`; `livewire/chat/*` (+partials), `livewire/memos/*`, `livewire/documents/*`, `livewire/admin/*` (dashboard/users/usage/errors/documents/knowledge/accounts), Volt auth pages, `components/admin/*` (design system: sidebar/table/tabs/kpi-card/badge/filter/empty-state), `layouts/` (app/admin/guest/auth-canvas), `emails/`.
...
- **Presentasi:** `PresentationWorkspace` → `PresentationGenerationService` → python `/api/presentations/generate` (PPTX + `PresentationVersion`). Edit via OnlyOffice Slides (`documentType=slide`) dengan `editorConfig()` (signed file URL + oo_token + JWT), persist edit lewat `PresentationCallbackController` (key segar + trusted URL + anti-replay + validasi PPTX). Download PPTX/PDF (`download`/`export-pdf`) memicu force-save (`force-save`) dulu agar memakai versi aktif terbaru.
