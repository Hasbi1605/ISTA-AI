...
- **Presentasi:** `PresentationWorkspace` → `PresentationGenerationService` → python `/api/presentations/generate` (PPTX + `PresentationVersion`). UI memakai shell 3 panel: sidebar riwayat, konfigurasi tengah, panel kanan permanen untuk empty/loading/stale/error/editor. Presentasi ready otomatis membuka OnlyOffice Slides (`documentType=slide`) dengan `editorConfig()` (signed file URL + oo_token + JWT); pending/processing lama (>10 menit tanpa update) diberi recovery action untuk redispatch job. Persist edit lewat `PresentationCallbackController` (key segar + trusted URL + anti-replay + validasi PPTX). Download PPTX/PDF (`download`/`export-pdf`) memicu force-save (`force-save`) dulu agar memakai versi aktif terbaru.
